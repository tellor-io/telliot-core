...
from telliot.apps.config import ConfigOptions, ConfigFile
...
class TelliotConfig(Base):
    ...
    def __init__(self, config_dir: Optional[Path] = None) -> None:
        main_file = ConfigFile(name='main', config_type=MainConfig, config_format='yaml', config_dir=config_dir)
        ep_file = ConfigFile(name='endpoints', config_type=EndpointList, config_format='yaml', config_dir=config_dir)
        chain_file = ConfigFile(name='chains', config_type=ChainList, config_format='json', config_dir=config_dir)

        super().__init__(main=main_file.get_config(),
