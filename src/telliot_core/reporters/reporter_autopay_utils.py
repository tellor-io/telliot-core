...
async def get_feed_details(query_id: str, autopay: TellorFlexAutopayContract) -> Optional[int]:
    assert isinstance(autopay, TellorFlexAutopayContract)
    current_time = TimeStamp.now().ts
    ...
    if tips_total > 0:
        logger.info(f"{query_ids_in_catalog[query_id]} has potentially {tips_total/1e18} in tips")

    return tips_total


async def autopay_suggested_report(autopay: TellorFlexAutopayContract) -> Tuple[Optional[str], Optional[int]]:
    chain = autopay.node.chain_id

    if chain in (1, 4, 137, 80001):
        assert isinstance(autopay, TellorFlexAutopayContract)
        datafeed_dict = {j: await get_feed_details(i, autopay) for i, j in query_ids_in_catalog.items()}
        datafeed_suggestion = {i: j for i, j in datafeed_dict.items() if j}
        datafeed_tips_sorted = sorted(datafeed_suggestion.items(), key=lambda item: item[1], reverse=True)
        ...
        if datafeed_suggested_qtag == single_suggested_qtag:
            return datafeed_suggested_qtag, (datafeed_suggested_tip + single_suggested_tip)  # type: ignore
